    ...
    <div style={{ background: "#F7F3EA", borderRadius: 20 }} className="p-5 shadow-xl">
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        <button
          ...
          className="shrink-0 flex items-center gap-1 rounded-full px-3 py-1.5 text-sm font-semibold"
        >
          <Plus size={15} /> Идея
        </button>
      </div>
          ...
          {ideas.map((idea) => (
            <IdeaCard
              ...
              onVote={() => wrap(idea.id, async () => {
                ...
                setIdeas((prev) => sortIdeas(prev.map((i) => (i.id === updated.id ? updated : i))));
              })}
              ...
              onSchedule={(dateId) => wrap(idea.id, async () => {
                ...
                setIdeas((prev) => sortIdeas(prev.map((i) => (i.id === result.idea.id ? result.idea : i))));
...
function IdeaCard({ idea, user, busy, onVote, onAddDate, onVoteDate, onSchedule, onDelete }) {
  ...
  return (
    <div style={{ borderColor: "#DCD4C0", background: "#FFFDF8" }} className="border rounded-2xl p-4">
      <div className="flex gap-3">
        <button
          ...
          className="shrink-0 w-14 rounded-xl flex flex-col items-center justify-center gap-0.5 py-2"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-1 shrink-0">
              {isAuthor && isOpen && (
                <button onClick={onDelete} disabled={busy} style={{ color: "#D8635B" }} className="p-1.5" title="Удалить">
                  <Trash2 size={14} />
                </button>
              )}
              <button onClick={() => setExpanded((v) => !v)} style={{ color: "#8B8FA0" }} className="p-1.5">
            ...
            <div className="flex flex-col gap-2">
              {idea.dates.map((d) => {
                ...
                return (
                  <div
                    key={d.id}
                    style={{ borderColor: "#DCD4C0", background: d.votedByMe ? "#E8A33D14" : "transparent" }}
                    className="border rounded-xl px-3 py-2.5 flex items-center gap-2"
                  >
                    <button
                      ...
                      className="shrink-0 min-w-10 rounded-lg px-2 py-1.5 text-xs font-bold flex items-center justify-center gap-1"
                    ...
                    {isOpen && (
                      <button
                        ...
                        className="shrink-0 rounded-full px-2.5 py-1.5 text-[11px] font-semibold flex items-center gap-1"
